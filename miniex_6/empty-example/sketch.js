...
function heartBeat() {
  ...
  if (heartX >= windowWidth/2-55 && heartX <= windowWidth/2-45) { //all of these set the heartbeat itself, the "mountains" in the middle, setting the Y-position due to certain x-positions
    ...
    heartY -= 8;
  } else if (heartX >= windowWidth/2+50 && heartX <= windowWidth/2+55) {
    heartY += 8;
  } else if (heartX >= windowWidth/2+55 && heartX <= windowWidth) {
...
class Star {
  ...
  shimmer () { //this is what makes the stars appear and disappear at random places - brightness is slowly increased and decresed ((even though it doesnt show very clearly))
    if (this.bri < 0) {
      this.x = random(width);
